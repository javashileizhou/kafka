...
abstract class AbstractFetcherThread(name: String, // 线程名字
                                     ...
                                     val brokerTopicStats: BrokerTopicStats) // broker主题监控指标 BrokerTopicStats's lifecycle managed by ReplicaManager
  ...
  private def maybeFetch(): Unit = {
    val fetchRequestOpt = inLock(partitionMapLock) {
      ...
      // 处理出错的分区，处理方式主要是将这个分区加入到有序的Map末尾，等待后续重试
  ...
  private def maybeTruncate(): Unit = {
    ...
    // 对于有Leader Epoch值的分区，将日志街区到Leader Epoch对应的位移处
    if (partitionsWithEpochs.nonEmpty) {
      truncateToEpochEndOffsets(partitionsWithEpochs)
    }
    // 对于没有Leader Epoch值的分区，将日志截取到高水位处
...
case class PartitionFetchState(fetchOffset: Long,
  ...
  // 分区处于截断中状态的条件，副本处于Truncating状态未被推迟执行
