...
class ControllerDeletionClient(controller: KafkaController, zkClient: KafkaZkClient) extends DeletionClient {
  ...
  // 取消/brokers/topics/<topic>节点节点数据变更的监听
 ...
 * @param controller
 */
class TopicDeletionManager(config: KafkaConfig, // 保存Broker端参数
